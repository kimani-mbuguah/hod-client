...
const backendApi = `${backendAPI}/member`
...
export const getCustomGroups = history => dispatch => {
  return axios
    .get(`${backendApi}/getgroups`)
    .then(res => {
      dispatch({
        type: CLEAR_ERRORS
      })
...
export const sendCustomGroupSms = messageBody => dispatch => {
  return axios
    .post(`${backendApi}/customgroupsms`, messageBody)
    .then(res => {
      dispatch({
        type: CLEAR_ERRORS
      })
